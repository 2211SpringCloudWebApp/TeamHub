...
<!DOCTYPE html>
	...
	<body>
		<div id="container">
			...
			<div id="subSideBar">
				...
				<ul>
				    <li style="color: #275ab5;"><a href="/approval/draftDocuments">기안 문서함</a></li>
			...
			<main>
				<div class="mainDiv">
				  ...
				  <ul class="progressNav">
				  	<li><a class="progressLink" href="/approval/draftDocuments?progress=전체">전체</a></li>
		 				...
		 				<tr align="center">
							<td colspan="5">
								<c:if test="${pi.currentPage - 1 != 0}">
									<a href="/approval/draftDocuments?page=1" class="naviBtn"> ◀️◀️ </a> &nbsp;&nbsp;
								</c:if>
								...
								<c:if test="${pi.currentPage + 1 <= pi.maxPage}">
									<a href="/approval/draftDocuments?page=${pi.maxPage }" class="naviBtn"> ▶️▶️ </a>
								</c:if>
							</td>
						</tr> 
						<tr>
	...
	/////////// progress 값에 따라 링크의 컬러 변경 ///////////////////
	var progress = "${progressMap.apprStatus}";
	var links = document.querySelectorAll('.progressLink'); // 모든 링크 선택
	for (var i = 0; i < links.length; i++) {
		var link = links[i];
		var linkText = link.textContent.trim(); // 링크 텍스트
		// 클릭한 링크는 블랙으로, 나머지는 회색으로 변경
		
		console.log("링크텍스트 : " + linkText);
		
		if (progress === linkText) {
